...
function inferDomainTags(input: {
  ...
  addIf("ai", ["machine learning", "deep learning", "llm", "rag", "embedding", /\bai\b/]);
  addIf("devtools", ["cli", "developer tool", "dx", "plugin", "sdk", "framework"]);
  addIf("infra", ["kubernetes", "k8s", "terraform", "helm", "observability", "prometheus"]);
  addIf("data", ["etl", "pipeline", "warehouse", "analytics", "bigquery", "snowflake"]);
  addIf("mobile", ["android", "ios", "react native", "flutter"]);
  addIf("web", ["web app", "frontend", "backend", "full stack", "full-stack"]);
  addIf("security", ["oauth", "jwt", "auth", "authentication", "authorization"]);
...
function extractTechStack(
  ...
  // Tooling detection
  if (fileNames.includes("dockerfile")) tooling.push("Docker");
  if (
    fileNames.includes("docker-compose.yml") ||
    fileNames.includes("docker-compose.yaml")
  )
    tooling.push("Docker Compose");
  if (fileNames.includes("webpack.config.js")) tooling.push("Webpack");
  if (
...
export const analysisRouter = createTRPCRouter({
  ...
  runPublic: candidateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (packageJsonFile && packageJsonFile.url) {
        try {
          ...
          if (
            ...
            console.log(
              "Successfully parsed package.json with dependencies:",
      ...
      // 3. Extract tech stack and signals
      const techStack = extractTechStack(languages, contentsArray, packageJson);
      const signals = detectSignals(contentsArray);
      const rationale = { confidence: 85, source: "GitHub API" };
